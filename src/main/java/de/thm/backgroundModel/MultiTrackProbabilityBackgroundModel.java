 ...
 */
public class MultiTrackProbabilityBackgroundModel extends BackgroundModel{
     ...
     * Contstructor
     */
    public MultiTrackProbabilityBackgroundModel(){}


    /**
     * Constructor for running sites against multiple intervals
     *
     * @param intervals - intervals to build background model against
     * @param sites - sites to search
     */
    public MultiTrackProbabilityBackgroundModel(List<Interval> intervals, Sites sites) throws Exception {

        IntersectCalculate calc = new IntersectCalculate();
        IntersectResult result = calc.searchSingleInterval(intervals.get(0),sites);

        positions.addAll(randPositionsScored(intervals.get(0), result));
